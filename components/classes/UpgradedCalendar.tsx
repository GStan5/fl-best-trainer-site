...
interface UpgradedCalendarProps {
  classes: ClassData[];
  onClassSelect: (classData: ClassData, element?: HTMLElement) => void;
  selectedClass?: ClassData | null;
}

export default function UpgradedCalendar({
  classes,
  onClassSelect,
  selectedClass,
}: UpgradedCalendarProps) {
  ...
  // Check if a class has Google Calendar integration enabled
  const hasGoogleCalendarIntegration = (classItem: ClassData) => {
    // This would check user settings or class configuration
    return true; // For testing, all classes have integration
  };
            ...
            <div className="block sm:hidden">
              {dayClasses.slice(0, 2).map((classItem, index) => (
                <motion.div
                  ...
                  className={`text-xs bg-gradient-to-r px-2 py-1 rounded text-white cursor-pointer truncate touch-manipulation relative
                    ${
                      classItem.location
                        .toLowerCase()
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {hasGoogleCalendarIntegration(classItem) && (
                      <FaGoogle className="text-xs text-blue-300" />
                    )}
                  </div>
                </motion.div>
              ))}
              ...
              {dayClasses
                ...
                .map((classItem, index) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: index * 0.05 }}
                    className={`text-xs bg-gradient-to-r px-1.5 py-0.5 rounded text-white cursor-pointer truncate touch-manipulation relative
                    ${
                      classItem.location
                        .toLowerCase()
                        .includes("bayfront park recreation center") ||
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-medium truncate pr-1">
                        {classItem.title.length > 10
                          ? `${classItem.title.substring(0, 10)}...`
                          : classItem.title}
                      </span>
    ...
    <div className="bg-black/40 backdrop-blur-lg rounded-xl border border-white/10 p-3 sm:p-4 md:p-6">
      ...
      <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-white/10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            <FaGoogle className="text-blue-300 text-sm" />
            <span className="text-white/80 text-xs">
              Google Calendar Integration
            </span>
          </div>
          <div className="flex items-center space-x-1 sm:space-x-2">
            <FaCalendarPlus className="text-green-300 text-sm" />
            <span className="text-white/80 text-xs">Auto Event Creation</span>
          </div>
          <div className="flex items-center space-x-1 sm:space-x-2">
            <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-yellow-400 rounded"></div>
            <span className="text-white/80 text-xs">Test Environment</span>
          </div>
        </div>
      </div>
    </div>
