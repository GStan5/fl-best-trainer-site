...
const generateWaiverPDF = async (data: WaiverBody) => {
  const doc = new jsPDF();
  ...
  doc.text("FL Best Trainer - Liability Waiver", pageWidth / 2, yPosition, { align: "center" });
  yPosition += lineHeight * 2;

  // Date
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  doc.text(`Date: ${new Date(data.date).toLocaleDateString()}`, margin, yPosition);
  yPosition += lineHeight * 2;
  ...
  doc.setFontSize(14);
  doc.setFont("helvetica", "bold");
  doc.text("LIABILITY WAIVER AND RELEASE", pageWidth / 2, yPosition, { align: "center" });
  yPosition += lineHeight * 2;
...
`;
  ...
  doc.setFont("helvetica", "normal");
  doc.text(`Signature Type: ${data.signatureType === 'draw' ? 'Hand-drawn' : 'Typed'}`, margin, yPosition);
  yPosition += lineHeight;

  if (data.signatureType === 'draw') {
    // For drawn signatures, we would add the image here
    doc.text("Electronic Signature: [Signature image attached]", margin, yPosition);
    yPosition += lineHeight;
    ...
    yPosition += lineHeight;
    doc.setFont("helvetica", "normal");
    doc.text("(Typed signature constitutes legal electronic signature)", margin, yPosition);
  }

  yPosition += lineHeight * 2;
  doc.text(`Date Signed: ${new Date(data.date).toLocaleDateString()} at ${new Date(data.date).toLocaleTimeString()}`, margin, yPosition);
  yPosition += lineHeight;
  doc.text(`IP Address: [Server Generated]`, margin, yPosition);
  yPosition += lineHeight;
